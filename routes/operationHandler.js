    ...
    // move mode
        ...
        try {
            picsPath.forEach(function (picPath) {
                ...
                if (picPath.indexOf('deleted_images') == -1) {
                    dirName = path.join(path.dirname(picPath).split('/').pop(), 'ManuallyDeleted');
                    tmpDir = conf.tmp_dir;
                } else {
                    var pieces = path.dirname(picPath).split('/');
                    dirName = pieces[pieces.length - 2];
                    tmpDir = conf.image_dir;
                }
                ...
                destPath = path.join(tmpDir, dirName, fileName);

                log('Dir Name -> ' + dirName);
